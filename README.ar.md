...
-   سام، يتطلب C++ 11
-   ESP32，要求C++17。需要更改`%LOCALAPPDATA%\Arduino15\packages\arduino\hardware\esp32\*.*.*\platform.txt`الكل فى`-std=gnu++11`ل`-std=gnu++17`
...
عندما تقوم بتضمين ملف الرأس هذا، فإنك تحصل تلقائيًا على cin وcout بناءً على ذلك`Serial`. See below for how to specify your own device. Here's an example sketch using `cin`و`cout`.
...
Printing of floats and doubles using `cout`يتجاهل محددات التنسيق.
