...
منذ[اردوينو STL الأصلي](https://github.com/mike-matera/ArduinoSTL)ليس لديه تحديثات أو منذ فترة طويلة (202203~202212)، قمت بإطلاق هذا الفرع لمكتبة Arduino العامة وأعدت تسميته Cpp_Standard_Library لتمييزه.

تحاول هذه المكتبة تنفيذ معظم وظائف المكتبة القياسية C++11~17 (STL) على Arduino. بالإضافة إلى ArduinoSTL، تشير بعض التطبيقات الوظيفية لهذه المكتبة أيضًا إلى MSVC وLLVM وboost. نظرًا لأن الواجهات مصنوعة وفقًا للمحكمة الخاصة بلبنان (STL)، ليست هناك حاجة لكتابة وثائق إضافية، ويمكنك الرجوع إلى أي وثيقة رسمية للمحكمة الخاصة بلبنان (STL). ما لم تتم الإشارة إلى خلاف ذلك، يجب أن يكون استخدام هذه المكتبة وسلوكها هو نفسه؛ إذا كان مختلفًا، فيجب أن يكون خطأً ونرحب بتقديم مشكلة.

إذا كنت بحاجة إلى بعض الميزات المتوفرة في المكتبة القياسية ولكنها غير متوفرة بعد في هذه المكتبة، فنحن نرحب أيضًا بإرسال إصدار، وسيعطي المؤلف الأولوية لتنفيذه نيابةً عنك.

يتم دعم بنيات AVR وSAM حاليًا. لا توجد حاليًا أي خطط لدعم ESP32، حيث قدم ESP32 رسميًا مكتبة قياسية. يؤدي استخدام مكتبتين قياسيتين بالتوازي إلى حدوث الكثير من المشكلات الصعبة.

# ميزات التوقيع (لا تقتصر على هذا) ميزات التوقيع (لا تقتصر على هذا)
